  ...
  if (!selectedSubtopic) {
    return (
      <div className="ctp-landing">
        ...
        {/* Cards Row: Trainer + Batch */}
        <div className="ctp-cards-row">
          {/* Trainer Card */}
          <motion.div 
            className="ctp-info-card ctp-trainer-card"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
          >
            <div className="ctp-card-icon-header">
              <div className="ctp-card-icon-circle ctp-icon-blue">
                <User size={20} />
              </div>
              <h3>Trainer Details</h3>
            </div>
            <div className="ctp-trainer-info">
              <div className="ctp-trainer-avatar">
                {metadata.trainer.name.charAt(0)}
              </div>
              <div className="ctp-trainer-details">
                <h4 className="ctp-trainer-name">{metadata.trainer.name}</h4>
                <p className="ctp-trainer-role">{metadata.trainer.role}</p>
              </div>
            </div>
            <div className="ctp-trainer-stats">
              <div className="ctp-stat-item">
                <span className="ctp-stat-label">Experience</span>
                <span className="ctp-stat-value">{metadata.trainer.experience}</span>
              </div>
              <div className="ctp-stat-item">
                <span className="ctp-stat-label">Specialization</span>
                <span className="ctp-stat-value">{metadata.trainer.specialization}</span>
              </div>
            </div>
          </motion.div>
          ...
          {/* Batch Card */}
          <motion.div 
            className="ctp-info-card ctp-batch-card"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
          >
            <div className="ctp-card-icon-header">
              <div className="ctp-card-icon-circle ctp-icon-green">
                <Calendar size={20} />
              </div>
              <h3>Course Details</h3>
            </div>
            <div className="ctp-batch-grid">
              <div className="ctp-batch-item">
                <BookOpen size={15} className="ctp-batch-icon" />
                <div>
                  <span className="ctp-batch-label">Course ID</span>
                  <span className="ctp-batch-value">{metadata.batch.id.replace('CR', 'CID')}</span>
                </div>
              </div>
              <div className="ctp-batch-item">
                <Hash size={15} className="ctp-batch-icon" />
                <div>
                  <span className="ctp-batch-label">Batch ID</span>
                  <span className="ctp-batch-value">{metadata.batch.id.replace('CR', 'BID')}</span>
                </div>
              </div>
              <div className="ctp-batch-item">
                <Clock size={15} className="ctp-batch-icon" />
                <div>
                  <span className="ctp-batch-label">Timing</span>
                  <span className="ctp-batch-value">{metadata.batch.timing}</span>
                </div>
              </div>
              <div className="ctp-batch-item">
                <Calendar size={15} className="ctp-batch-icon" />
                <div>
                  <span className="ctp-batch-label">Start Date</span>
                  <span className="ctp-batch-value">{metadata.batch.startDate}</span>
                </div>
              </div>
              <div className="ctp-batch-item">
                <Monitor size={15} className="ctp-batch-icon" />
                <div>
                  <span className="ctp-batch-label">Duration • Mode</span>
                  <span className="ctp-batch-value">{metadata.batch.duration} • {metadata.batch.mode}</span>
                </div>
              </div>
            </div>
          </motion.div>
