...
export const trainerRegister = async (req, res) => {
  try {
    ...
    const trainerSnapshot = await trainersRef.orderByChild("email").equalTo(email.trim().toLowerCase()).once("value");
    ...
    const tempKey = `trainer_${sanitizedEmail}`;
    ...
    await sendEmail({
      ...
      subject: "Registration OTP for Trainer",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
          <h2 style="color: #1a73e8;">Trainer Registration OTP</h2>
          <p>Hi <strong>${resolvedName.trim()}</strong>,</p>
          <p>Your OTP for completing trainer registration is:</p>
